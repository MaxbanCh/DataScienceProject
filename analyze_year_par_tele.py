...
file_path = current_dir / "ressources" / "20190308-tv-years(2).csv"

if not file_path.exists():
    print(f"ERREUR : Fichier CSV introuvable : {file_path}")
    raise SystemExit(1)

graph_dir = current_dir / "graphes" / "year_par_tele"
...
plt.title("Évolution annuelle du taux de parole des femmes")
plt.xlabel("Année")
plt.ylabel("Taux de parole (médiane)")
plt.legend()
plt.tight_layout()
plt.savefig(graph_dir / "tele_trend_global_public_prive.png", dpi=300)
plt.show()
...
plt.title("Écart du taux de parole (public - privé)")
plt.xlabel("Année")
plt.ylabel("Écart (points)")
plt.tight_layout()
plt.savefig(graph_dir / "tele_gap_public_prive.png", dpi=300)
plt.show()

# Top 10 chaînes sur la période
...
plt.title("Top 10 chaînes (moyenne sur la période)")
plt.xlabel("Taux de parole (moyenne)")
plt.ylabel("Chaîne")
plt.tight_layout()
plt.savefig(graph_dir / "tele_top10_channels.png", dpi=300)
plt.show()

# Heatmap année x chaîne (top 10)
...
plt.title("Heatmap (Top 10 chaînes)")
plt.xlabel("Chaîne")
plt.ylabel("Année")
plt.tight_layout()
plt.savefig(graph_dir / "tele_heatmap_top10.png", dpi=300)
plt.show()

# 4) Résumé texte
summary_lines = [
    "Résumé year_par_tele.csv",
    ...
    "",
    "Top 10 chaînes (moyenne):",
]
summary_lines += [f"- {name}: {value:.2f}" for name, value in channel_means.items()]

with open(analysis_dir / "tele_summary.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(summary_lines))

print("✅ Graphes enregistrés dans /graphes/year_par_tele")
print("✅ Résumé enregistré dans /analyses/tele_summary.txt")

# 5) Régression (taux par chaîne et année)
df_long = df.melt(
    id_vars=["year"],
...
with open(analysis_dir / "tele_regression_female_rate.txt", "w", encoding="utf-8") as f:
    f.write("=== OLS COMPLET -> female_rate ~ year + channel ===\n")
    f.write(model.summary().as_text())
    f.write("\n")

print("✅ Régression enregistrée dans /analyses/tele_regression_female_rate.txt")
