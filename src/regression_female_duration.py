...
OUT_DIR = RESULTS / "regression_female_duration"
...
def fit_ols_like_r(df, y_col, feature_cols, name, test_size=0.2):
...
# Ridge 
# -----------------------------
def fit_ridge_with_diagnostics(df, y_col, feature_cols, name, alpha=1.0, test_size=0.2):
    X = df[feature_cols].copy()
    y = df[y_col].astype(float).copy()
...
def main():
    ...
    y_col = "female_duration"
    df = df.dropna(subset=[y_col]).copy()

    # M1 : OLS 
    features_m1 = [
        "male_duration", "music_duration",
    ...
    fit_ols_like_r(df, y_col, features_m1, name="M1_ols")

    # M2 : Ridge sans male_duration 
    features_m2 = [c for c in features_m1 if c != "male_duration"]
    fit_ridge_with_diagnostics(df, y_col, features_m2, name="M2_ridge_no_male", alpha=1.0)
